...
test('country_city_taxonomy: covers 65 countries with valid taxonomy schema', () => {
  const countryKeys = Object.keys(COUNTRY_CITY_MAP);
  assert.equal(countryKeys.length, 65, `Expected 65 countries, found ${countryKeys.length}`);

  // Test newly added critical countries
...
test('country_city_taxonomy: TECH_LOCATIONS aggregates almost 400 unique tech hubs', () => {
  assert.ok(Array.isArray(TECH_LOCATIONS));
  assert.ok(TECH_LOCATIONS.length >= 380, `Expected >= 380 locations, got ${TECH_LOCATIONS.length}`);

  // Verify key cities are included
...
test('REGIONAL_MARKET_MAP: contains new hubs in asia_pacific, europe, and middle_east_africa', () => {
  // Asia Pacific
...
test('matchLocationString: matches cities and bidirectional alias expansions', () => {
  ...
  // Bidirectional aliases (Kyiv <-> Kiev)
